...
// =============================
export function computeLikeOpacity(x) {
  // Fade in when dragging right past 40px, full at +120px
  if (x <= 40) return 0;
  return Math.min((x - 40) / 80, 1);
}

export function computeNopeOpacity(x) {
  // Fade in when dragging left past -40px, full at -120px
  if (x >= -40) return 0;
  return Math.min((-x - 40) / 80, 1);
}

export function computeRotate(x) {
  // Rotate the card like a hinge
  return x * 0.1; // degrees
}

export function toggleId(arr, id) {
...
if (
  ...
  window.__RUN_CAMPUSWIPE_TESTS__ !== false
) {
  runDevTests();
}
...
function EventCard({ e }) {
  return (
    <Card className="overflow-hidden rounded-2xl shadow-md">
      <div className="h-48 w-full bg-gray-100 overflow-hidden dark:bg-neutral-800">
        <img
          src={e.img}
          alt={e.title}
      ...
      <CardContent className="text-sm text-muted-foreground space-y-2">
        ...
        <p className="text-slate-600 dark:text-neutral-300">{e.description}</p>
      </CardContent>
    </Card>
  );
}
...
export default function CampusSwipe() {
  ...
  const filtered = useMemo(() => {
    const q = query.trim();
    return MOCK_EVENTS.filter((e) => eventMatches(e, q, selectedClubs));
  }, [query, selectedClubs]);

  // Like helpers
  const like = (id) => setLiked((s) => (s.includes(id) ? s : [...s, id]));
  const unlike = (id) =>
    setLiked((s) => s.filter((x) => x !== id));
  const toggleLike = (id) => setLiked((s) => toggleId(s, id));
  ...
  const leaveEvent = (id) => setJoined((s) => s.filter((x) => x !== id));
  const toggleJoin = (id) => setJoined((s) => toggleId(s, id));

  // Pass helper
  const nope = (id) => setPassed((s) => Array.from(new Set([...s, id])));

  const toggleClub = (c) => setSelectedClubs((s) => toggleId(s, c));
  const clearClubs = () => setSelectedClubs([]);

  return (
    <div className={theme === 'dark' ? 'dark' : ''}>
      <div className="min-h-screen w-full bg-gradient-to-b from-white to-slate-50 dark:from-black dark:to-neutral-900">
        {/* Top Bar */}
        ...
        <div className="mx-auto grid max-w-7xl gap-6 p-4 lg:grid-cols-12 text-slate-900 dark:text-neutral-100">
          {/* Left: Swipe Deck */}
          <div className="lg:col-span-5">
            <div className="relative h-[560px]">
              ...
              {top ? (
                ...
                <div className="flex h-full items-center justify-center rounded-2xl border bg-white dark:bg-neutral-900 dark:border-neutral-800">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {selectedClubs.length > 0 && (
                    <Button variant="ghost" size="sm" onClick={clearClubs}>
                      zurücksetzen
                    </Button>
                  )}
                </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        <Button
                          ...
                          onClick={() =>
                            joined.includes(e.id) ? leaveEvent(e.id) : joinEvent(e.id)
                          }
                        >
                          {joined.includes(e.id) ? (
                            <>
                              <Check className="h-4 w-4 mr-1" /> Beigetreten
                            </>
                          ) : (
                            "Beitreten"
                          )}
                        ...
                        <Button
                          ...
                          aria-label={
                            liked.includes(e.id) ? "Like entfernen" : "Like hinzufügen"
                          }
            ...
            <Tabs defaultValue="likes">
              ...
              <TabsContent value="joined" className="pt-3">
                <div className="flex flex-wrap gap-2">
                  {joined.length === 0 && (
                    <div className="text-sm text-muted-foreground">
                      Noch keine Zusagen – beitreten über Freunde oder Suche.
                    </div>
                  )}
                  {joined.map((id) => {
                    const e = MOCK_EVENTS.find((x) => x.id === id);
                    return (
                      ...
                      >
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          aria-label="Zusage zurückziehen"
              ...
              <TabsContent value="export" className="pt-3">
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      const webhook = window.__N8N_WEBHOOK__ || "";
                      if (!webhook) {
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Nutze einen n8n Webhook (HTTP Request Trigger), um Likes in eine DB oder Google
                  Sheet zu schreiben.
                </p>
              </TabsContent>
              <TabsContent value="analytics" className="pt-3">
                <div className="text-sm text-muted-foreground">
                  Für Grafana/Prometheus: Sende Metriken ("likes_total", "swipes_total") an einen
                  Pushgateway und erstelle ein Dashboard. Alternativ: schreibe in eine
                  Timeseries-DB (z. B. InfluxDB).
                </div>
              </TabsContent>
            </Tabs>
          </div>
...
function MenuSheet({
  ...
  return (
    <Sheet>
      ...
      <SheetContent
        side="right"
        className="w-[360px] sm:w-[420px] dark:bg-neutral-900 dark:border-neutral-800"
      >
        <SheetHeader>
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  const ok = window.confirm('Are you sure about that?');
                  if (ok) onChangeTheme('light');
                }}
              >
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Light (weiß) erfordert Bestätigung.
            </p>
          </div>
          <div className="rounded-xl border p-3 dark:border-neutral-800">
            <div className="text-sm font-medium mb-2">Debug</div>
            <div className="text-xs text-muted-foreground">
              Likes: {JSON.stringify(likes)} | Joined: {JSON.stringify(joined)} | Passed: {JSON.stringify(passed)}
            </div>
          </div>
        </div>
      </SheetContent>
