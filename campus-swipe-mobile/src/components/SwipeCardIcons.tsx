...
export const SwipeCard: React.FC<Props> = ({ event, onSwipe, onSwipeUp }) => {
  ...
  const likeOpacityStyle = useAnimatedStyle(() => ({ opacity: interpolate(translateX.value, [0, SWIPE_THRESHOLD], [0, 1]) }));
  const nopeOpacityStyle = useAnimatedStyle(() => ({ opacity: interpolate(translateX.value, [-SWIPE_THRESHOLD, 0], [1, 0]) }));
  const upOpacityStyle = useAnimatedStyle(() => ({ opacity: interpolate(translateY.value, [-SWIPE_THRESHOLD, -SWIPE_THRESHOLD / 2, 0], [1, 0.5, 0]) }));
